...
void loop() { 
 ...
 //loop only runs if the button is in on state,  if not it does not fo blinking and if in -between the button is pressed, it stops the loop
while(digitalRead(D5)!= LOW){ 
//if mothion deteccted and it is the first time when motion is gtting detected, then start blinking
  if(bPIR == HIGH && lightState == HIGH){
  ...
  lightState = LOW;                     //state to keep a recoed that motion has already been detected 1 time
